pipeline {
  ...
  environment {
    ...
    BUILD_VERSION_ARG = 'REDIS_VERSION'
    IG_USER = 'imagegenius'
    IG_REPO = 'docker-redis'
    CONTAINER_NAME = 'redis'
    DIST_IMAGE = 'alpine'
    DIST_TAG = '3.18'
    DIST_REPO = 'http://dl-cdn.alpinelinux.org/alpine/v3.18/main/'
    DIST_REPO_PACKAGES = 'redis'
    MULTIARCH = 'true'
    CI = 'true'
    CI_WEB = 'false'
    CI_PORT = ''
      ...
      steps {
        ...
        sh '''#! /bin/bash
              ...
              docker buildx build \
                ...
                --label \"org.opencontainers.image.url=https://github.com/imagegenius/docker-redis/packages\" \
                --label \"org.opencontainers.image.source=https://github.com/imagegenius/docker-redis\" \
                ...
                --label \"org.opencontainers.image.description=Redis is an in-memory database that persists on disk. The data model is key-value, but many different kind of values are supported: Strings, Lists, Sets, Sorted Sets, Hashes, Streams, HyperLogLogs, Bitmaps.\" \
